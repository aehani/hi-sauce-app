...
export const handler = async (event) => {
  ...
  // GET = browser redirect after payment complete
...
<head>
  <meta charset="utf-8">
  <title>Payment Complete — Hi! Sauce</title>
...
<body>
  ...
  // POST = payment notification from processor
  let orderId, transactionId, status

  try {
    const data = JSON.parse(event.body || '{}')

    // MX Merchant notification format
    if (data.PONumber || data.poNumber) {
      orderId       = data.PONumber || data.poNumber
      transactionId = data.transactionId || data.id || data.receiptNumber
      status        = (data.status === 'Approved' || data.approved === true || data.responseCode === '00') ? 'paid' : 'failed'
    }
    // Authorize.net webhook format
    else if (data.payload || data.eventType) {
    ...
    // Form-encoded (Authorize.net silent post)
    const params  = new URLSearchParams(event.body || '')
    orderId       = params.get('x_invoice_num') || params.get('PONumber')
  ...
  if (orderId && status === 'paid') {
    try {
      ...
      if (SUPABASE_URL && SUPABASE_ANON_KEY) {
        ...
        console.log('Supabase update:', res.status, 'for order:', orderId)
      }
    } catch (err) {
